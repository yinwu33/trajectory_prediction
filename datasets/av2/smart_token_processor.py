...
class TokenProcessor:
    ...
    def __init__(self, token_size, agent_token_path: str | None = None, map_token_path: str | None = None):
        self.agent_token_path = self._resolve_token_path(
            override_path=agent_token_path,
            filename=f'cluster_frame_5_{token_size}.pkl',
        )
        self.map_token_traj_path = self._resolve_token_path(
            override_path=map_token_path,
            filename='map_traj_token5.pkl',
        )
        self.noise = False
        self.disturb = False
        self.shift = 5
    ...
    def _resolve_token_path(self, override_path: str | None, filename: str) -> str:
        ...
        local_candidate = (
            Path(__file__).resolve().parents[3] / "models" / "smart" / "tokens" / filename
        )
        if local_candidate.is_file():
            return str(local_candidate)

        raise FileNotFoundError(f"Could not resolve SMART token file: {filename}")
