...
from models.qcnet.utils import wrap_angle


class TargetBuilder(BaseTransform):
    def __init__(self, num_historical_steps: int, num_future_steps: int) -> None:
    ...
    def __call__(self, data: HeteroData) -> HeteroData:
        ...
        rot_mat[:, 1, 0] = sin
        rot_mat[:, 1, 1] = cos
        data["agent"]["target"] = origin.new_zeros(
            data["agent"]["num_nodes"], self.num_future_steps, 4
        )
        data["agent"]["target"][..., :2] = torch.bmm(
            data["agent"]["position"][:, self.num_historical_steps :, :2]
