...
from models.qcnet.utils import safe_list_index
from models.qcnet.utils import side_to_directed_lineseg
...
class AV2QCNetDataset(Dataset):
    ...
    Args:
        root (string): dataset root.
        split (string): split name, e.g. `"train"` | `"val"` | `"test"` | `"mini_train"`.
        raw_dir (string, optional): path of raw logs. By default, path/to/root/split.
        processed_dir (string, optional): optionally specify the directory of the processed data. By default, the
            processed directory is path/to/root/cache/av2_qcnet/split/. If specified, the path of the processed .pkl files is
    ...
    def __init__(self,
        ...
        if processed_dir is None:
            processed_dir = os.path.join(root, 'cache', 'av2_qcnet', split)
        ...
        super(AV2QCNetDataset, self).__init__(root=root, transform=transform, pre_transform=None, pre_filter=None)
    ...
    def _process(self) -> None:
        if os.path.isdir(self.processed_dir) and len(self.processed_file_names) == len(self.raw_file_names):
            return
